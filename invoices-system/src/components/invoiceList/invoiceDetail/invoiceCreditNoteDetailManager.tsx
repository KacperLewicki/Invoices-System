...
const CreditNoteDetails: React.FC = () => {
    ...
    if (!creditNote) {
        ...
        return <p className="text-center mt-10 text-lg text-gray-600">Nota kredytowa nie została znaleziona.</p>;
    }
            ...
            <div className="w-full max-w-7xl bg-white p-8 shadow-lg rounded-lg border border-gray-200">
                ...
                <section className="mb-6 text-gray-900">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <p><strong>Klient:</strong> {creditNote.customerName}</p>
                        <p><strong>Sprzedawca:</strong> {creditNote.seller}</p>
                        <p><strong>Metoda płatności:</strong> {creditNote.paymentMethod}</p>
                        <p><strong>Miesiąc efektywności:</strong> {creditNote.effectiveMonth}</p>
                        <p><strong>Waluta:</strong> {creditNote.currency}</p>
                    </div>
                </section>

                <section className="mb-6 text-gray-900">
                    <h2 className="text-lg font-semibold mb-2 text-purple-700">Szczegóły Noty Kredytowej</h2>
                    <div className="grid grid-cols-2 gap-4">
                        <p><strong>Data wystawienia:</strong> {new Date(creditNote.dataInvoice).toLocaleDateString()}</p>
                        <p><strong>Data sprzedaży:</strong> {new Date(creditNote.dataInvoiceSell).toLocaleDateString()}</p>
                        <p><strong>Termin płatności:</strong> {new Date(creditNote.dueDate).toLocaleDateString()}</p>
                        <p><strong>Komentarz:</strong> {creditNote.comments || "Brak"}</p>
                    </div>
                </section>

                <section className="mb-6 text-gray-900">
                    <h2 className="text-lg font-semibold mb-2 text-purple-700">Pozycje Noty Kredytowej</h2>
                ...
                <section className="mt-6 text-right border-t border-gray-300 pt-4 text-gray-900">
                    <p className="text-lg"><strong>Razem Netto:</strong> {creditNote.summaryNetto} {creditNote.currency}</p>
                    <p className="text-lg"><strong>Razem VAT:</strong> {creditNote.summaryVat} {creditNote.currency}</p>
                    <p className="text-lg font-bold text-purple-700"><strong>Razem Brutto:</strong> {creditNote.summaryBrutto} {creditNote.currency}</p>
                </section>
            </div>
        </div>
    );
